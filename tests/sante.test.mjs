...
import { finessUrl, gpUrl, nearbyFiness, parseGps, santeView, urgenceKind } from '../src/lib/sante.ts';
...
const fakeGet = (place) => {
  const calls = [];
  const get = async (url) => {
    const kind = url.includes('categ_code') ? 'pharmacie' : 'urgences';
    calls.push(url);
    const n = calls.filter((u) => u.includes('categ_code') === (kind === 'pharmacie')).length;
    return fixture(`sante-finess-${kind}-${place}-${n === 1 ? 'proche' : 'large'}.json`);
  };
  return { get, calls };
};
...
const load = async (place, p) => {
  ...
  const ph = await nearbyFiness('pharmacie', p, get), urg = await nearbyFiness('urgences', p, get);
  const view = santeView({
    pharmacies: ph.places, urgences: urg.places, finessDate: ph.date,
    gps: parseGps(fixture(`sante-ameli-generalistes-${place}.json`), p),
    ameliDate: fixture('sante-ameli-meta.json').metas.default.modified,
  });
  return { view, calls };
};

test('builds a bounding box around the point', () => {
  const q = new URL(finessUrl('pharmacie', SEGUR, 1.5)).searchParams;
  ...
  assert.equal(new URL(finessUrl('urgences', SEGUR, 15)).searchParams.get('san_urg__exact'), 'true');
...
test('GPs: one row per time slot is collapsed to one per doctor and address', () => {
  const raw = fixture('sante-ameli-generalistes-segur.json').results;
...
test('emergency kinds come from the name and the medicine flag', () => {
  assert.equal(urgenceKind({ rs: 'ANTENNE SMUR MODANE', san_med: false }), 'smur');
  assert.equal(urgenceKind({ rs: 'CHRU DE TOURS - ANTENNE DE LOCHES', san_med: false }), 'smur');
  assert.equal(urgenceKind({ rs: 'GHU APHP NUP SITE ROBERT DEBRE', san_med: true }), 'pediatrique');
  assert.equal(urgenceKind({ rs: 'CHRU TROUSSEAU - CHAMBRAY', san_med: true }), 'generale');
  assert.equal(urgenceKind({ rs: 'CH SUD FRANCILIEN', san_med: false }), 'generale');
});

test('partial failure: the rest is shown with a note', () => {
  const view = santeView({ pharmacies: undefined, gps: [], urgences: [] });
