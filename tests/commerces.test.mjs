...
import { commercesView } from '../src/lib/commerces.ts';
...
test('Nancy, 35 rue Joseph Mougin: a bakery close by, post office and bank a little further', async () => {
  ...
  assert.equal(v.facts[0].value, 'Boulangerie à 90 m');
  assert.equal(v.facts[0].detail, 'Supérette ou épicerie à 530 m, supermarché ou hypermarché à 620 m.');
  assert.equal(v.facts[1].value, 'À 650 m');
  assert.equal(v.facts[2].value, 'À 650 m');
...
test('Saint-Véran: a small shop in the village, the supermarket and the bank far down the valley', async () => {
  ...
  assert.equal(v.facts[0].value, 'Supérette ou épicerie à 630 m');
  assert.equal(v.facts[1].value, 'À 810 m');
  assert.equal(v.facts[2].value, 'À 17 km');
...
test('Fort-de-France: many shops around', async () => {
  ...
  assert.equal(v.facts[0].value, 'Boulangerie à 130 m');
  assert.equal(v.facts[2].value, 'À 40 m');
