...
import { urls, rapportUrl, argilesFact, inondationFact, communeFact, sitesItems } from '../src/lib/risques.ts';
...
test('builds v1 URLs with longitude first and both district and commune codes', () => {
  ...
  assert.match(u.sismique, /zonage_sismique\?code_insee=75107%2C75056$/);
...
test('links to the public Géorisques report for the address', () => {
  const u = new URL(rapportUrl(segur));
...
test('clay: medium exposure is a warning', () => {
  const f = argilesFact(fixture('risques-archive-rga-saint-malo.json'), malo);
...
test('clay: strong is an alert, weak is info', () => {
  assert.equal(argilesFact({ codeExposition: '3', exposition: 'Exposition forte' }, malo).level, 'alert');
  assert.equal(argilesFact({ codeExposition: '1', exposition: 'Exposition faible' }, malo).level, 'info');
});

test('clay: no result is ok, except in Paris which the map does not cover', () => {
  assert.equal(argilesFact(null, malo).level, 'ok');
  assert.equal(argilesFact({}, malo).level, 'ok');
  const paris = argilesFact(null, segur);
  assert.equal(paris.level, 'unknown');
  assert.match(paris.detail, /Paris/);
});

test('flood: point inside a TRI lists its scenarios', () => {
  const f = inondationFact(fixture('risques-archive-schema-tri-zonage.json'));
  assert.equal(f.level, 'warn');
...
test('commune: seismic zone 2 and radon zone 1', () => {
  const f = communeFact(fixture('risques-archive-zonage-sismique-saint-malo.json'), fixture('risques-archive-radon-saint-malo.json'));
...
test('commune: the highest level wins, radon zone 3 is a warning', () => {
  const sism = { data: [{ code_zone: '1' }] };
  const radon = { data: [{ classe_potentiel: '3' }] };
  const f = communeFact(sism, radon);
  assert.equal(f.level, 'warn');
...
test('commune: one missing source is said, both missing is unknown', () => {
  const f = communeFact(undefined, fixture('risques-archive-radon-saint-malo.json'));
...
test('error payloads throw', () => {
  const err = fixture('risques-archive-schema-erreur.json');
  assert.throws(() => inondationFact(err), /Paramètres/);
  assert.throws(() => communeFact(err, err), /Paramètres/);
  assert.throws(() => sitesItems(err, undefined, segur), /Paramètres/);
  assert.throws(() => argilesFact({ code: 'INTERNAL', message: 'boom', timestamp: 'x' }, malo), /boom/);
});

test('sites: polluted sites and ICPE, sorted by distance, "Non ICPE" dropped', () => {
  const orleans = { lat: 47.8944, lon: 1.9525 };
  const { items, more } = sitesItems(fixture('risques-archive-schema-ssp.json'), fixture('risques-archive-installations-classees-page1.json'), orleans);
...
test('sites: polluted site kinds, fiche link and nameless entries', () => {
  const { items } = sitesItems(fixture('risques-archive-schema-ssp.json'), undefined, segur);
  const usine = items.find((i) => i.name === 'Ancienne usine (exemple)');
  assert.match(usine.detail, /Pollution des sols suivie par l’administration/);
  assert.ok(usine.distance < 400);
