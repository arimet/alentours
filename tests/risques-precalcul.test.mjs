...
import { parseCsv, build } from '../scripts/risques.mjs';
import { communeOf, gasparFacts, risquesView } from '../src/lib/risques.ts';

// Extract of public/data/risques/75.json and 05.json (GASPAR 21/09/2026, radon 2019).
const file = JSON.parse(readFileSync(new URL('fixtures/risques-commune-extrait.json', import.meta.url)));
...
test('gaspar facts: DDRM risks with plans, then CatNat', () => {
  const [risques, catnat] = gasparFacts(communeOf(file, veran));
  assert.equal(risques.value, 'Inondation, Mouvement de terrain, Séisme, Avalanche, Feu de forêt');
  assert.match(risques.detail, /Plan de prévention des risques naturels : PPRN-Multi - Saint-Véran 2018 \(approuvé\)/);
...
test('all live calls fail: commune facts only, said out loud', () => {
  const v = risquesView(down, file, veran);
...
test('everything fails: the block errors as before', () => {
  assert.throws(() => risquesView(down, undefined, veran), /Géorisques ne répond pas/);
  assert.throws(() => risquesView(down, { _meta: {} }, veran), /Géorisques ne répond pas/);
});
...
test('live wins; the commune file fills the missing radon and adds GASPAR', () => {
  ...
  const v = risquesView(live, file, segur);
  ...
  const liveRadon = risquesView([...live.slice(0, 3), { value: { data: [{ classe_potentiel: '3' }] } }, ...live.slice(4)], file, segur);
...
test('live without file: unchanged behaviour, missing calls named', () => {
  const v = risquesView([{ value: null }, undefined, undefined, undefined, undefined, undefined], undefined, veran);
