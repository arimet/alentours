...
## 5. Téléchargements en masse (pour le pré-calcul)

Vérifié le 24/09/2026 via l'API data.gouv (`/api/1/datasets/…`) et des requêtes `HEAD`/`GET` sur les fichiers. Pendant ce temps, l'API Géorisques v1 était toujours en 503/500 ou sans réponse.

| Donnée | Jeu data.gouv | Fichier | Format, taille | Date | Licence | Remarques |
|---|---|---|---|---|---|---|
| **GASPAR** (risques par commune, AZI, PPRN/M/T, CatNat, DICRIM, TIM) | [Base GASPAR](https://www.data.gouv.fr/datasets/base-nationale-de-gestion-assistee-des-procedures-administratives-relatives-aux-risques-gaspar) (ministère de la Transition écologique) | `https://files.georisques.fr/GASPAR/gaspar.zip` (l'URL `http://` de data.gouv redirige en 301) | ZIP de 8,1 Mo, 8 CSV séparés par `;` (72 Mo décompressés) : `ddrm_risq_gaspar` (9 Mo, `cod_commune;lib_commune;lib_risque;num_risque`, 176 000 lignes), `catnat_gaspar` (42 Mo), `pprn_gaspar` (15 Mo), `azi`, `pprt`, `pprm`, `dicrim`, `tim` | fichiers datés du **21/09/2026** (`Last-Modified` identique) ; mise à jour data.gouv 19/08/2026 | Licence Ouverte (`fr-lo`) | Encodage mélangé : `catnat` contient du « Ã© » (UTF-8 encodé deux fois), à nettoyer. `ddrm_risq` liste les risques (dont « Séisme »), **pas la zone sismique**. |
| **Radon** par commune | [Connaître le potentiel radon de ma commune](https://www.data.gouv.fr/datasets/connaitre-le-potentiel-radon-de-ma-commune) (ASN, ex-IRSN) | `https://static.data.gouv.fr/resources/connaitre-le-potentiel-radon-de-ma-commune/20190506-174309/radon.csv` (et un shapefile de 33 Mo) | CSV `;` de 1,2 Mo, 36 093 communes : `nom_comm;nom_dept;insee_com;classe_potentiel;reg` | **06/05/2019** (communes de 2019 : les fusions depuis ne sont pas à jour) | Licence Ouverte (`fr-lo`) | Classes 1/2/3 = arrêté du 27 juin 2018 (26 937 / 2 081 / 7 075 communes). **Codes d'arrondissement** pour Paris, Lyon, Marseille (75107, 69381, 13201…), comme l'API. Le jeu BRGM « Zonage en potentiel radon » n'offre qu'un WMS. |
| **Zone sismique** par commune | Pas de jeu officiel national trouvé. Le seul national est [Zonage sismique de la France](https://www.data.gouv.fr/datasets/zonage-sismique-de-la-france-1), publié par **wpd** (entreprise éolienne, pas une source officielle) | `https://static.data.gouv.fr/resources/zonage-sismique-de-la-france-1/20180730-100725/France_zonage_sismique.zip` | ZIP shapefile de 129 Mo | 30/07/2018, d'après un Excel planseisme.fr/BRGM de janvier 2015 | Licence Ouverte (`fr-lo`) | Le reste, ce sont des jeux départementaux (DDT). Le texte qui fait foi est l'annexe de l'article D563-8-1 du code de l'environnement (liste des communes, Légifrance). planseisme.fr n'a pas répondu. **Conseil** : pré-calculer par l'API `zonage_sismique?code_insee=` (jusqu'à 10 codes par appel, donc ~3 500 appels à 5/s, soit ~12 min) quand elle remarchera, ou partir de l'annexe D563-8-1. |

Autres points utiles pour le pré-calcul :
- La carte des **argiles** a été mise à jour par l'arrêté du 9 janvier 2026 (applicable aux ventes depuis le 1er juillet 2026). Géorisques la propose en shapefile par département ([page](https://www.georisques.gouv.fr/donnees/bases-de-donnees/retrait-gonflement-des-argiles-version-2026)). Elle **ne couvre pas la ville de Paris**. On ne sait pas si l'API `/rga` sert la version 2020 ou 2026 (non vérifié).
- Le site Géorisques affichait le 24/09/2026 un bandeau « incident technique » sur la génération des rapports. La page `rapport2?form-adresse=true&lon=…&lat=…&codeInsee=…` répond bien 200, mais son contenu est construit en JavaScript et n'a pas été vérifié.
