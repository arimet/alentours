...
export const indexUrl = (insee: string) => `${WFS}?${new URLSearchParams({
...
// day is the worst of the sub-indices of NO2, O3, PM10, PM2.5 and SO2. Codes 0 (absent) and 7 ("événement", event) are
...
export type AtmoIndex = { date: string; code: number; label: string; zone: string; source: string; pollutants: string[] };

const list = (a: string[]) => a.length > 1 ? `${a.slice(0, -1).join(', ')} et ${a.at(-1)}` : a[0];

export const parseIndices = (json: any): AtmoIndex[] =>
  ...
  })).sort((a: AtmoIndex, b: AtmoIndex) => a.date.localeCompare(b.date));
...
const fact = (label: string, i: AtmoIndex): Fact => SCALE[i.code - 1]
...
export const airView = (indices: AtmoIndex[], today: string): BlockView => {
